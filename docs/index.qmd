  ...
  html:
    ...
    title-block-header: |
      <div style="text-align:center; margin-bottom: 1em;">
        <img src="project/coral_image_files/mir_logo.jpg" style="height: 100px; margin-right: 30px;">
        <img src="coral_image_files/noaa_crcp.jpg" style="height: 100px;">
      </div>
  pdf:
...
![Figure 1: Map of the Florida Reef Tract with Specified M:IR Restoration Sites](images/MIR%20Map.png){fig-align="center"}
...
source("R Scripts/MIR_make_size_bins.R")
source("R Scripts/theme_publication.R")
source("R Scripts/QuadPanel.R")
# source("R Scripts/bleaching_estimates.R")
# source("R Scripts/total_bleach_code.R")
source("R Scripts/dis_ble_prev_ratio_est.R")


setwd("~/Downloads/MIR-Krampitz/Project")
```
...
NCRMP_MIR <- read_csv("data/NCRMP_MIR_cora_2022_24_psu_grps.csv") %>%
...
DRM <- read_csv("data/DRM_cora_2022_24_psu_grps.csv") %>%
  group_by(YEAR, strat_cora, analysis_group) %>%
  summarise(count = n()) %>%
  filter(analysis_group != "NCRMP_NA") %>%
  rename(STRAT = strat_cora,
...
df <- read.csv( "data/FK2024_NCRMP_DRM_MIR_corsz_AR_NK.csv") %>%
  filter(N > 0,
...
names <- read_csv("data/DRM_FLK_2014_2024_2stage_coral_demographics.csv") %>% select(SPECIES_CD, SPECIES_NAME) %>% unique()
...
cvr <- read_csv("data/fk2022_24_NCRMP_MIR_bcov_catcd_dom_NK.csv")%>%
  dplyr::rename(
...
den <- read_csv("data/fk2022_24_NCRMP_MIR_corabd_dom_NK.csv") %>%
  rename(
    avDen = wavdns,
    SE = se_wdns,
    CV_perc = wcv_dns,
...
names <- read_csv("data/NCRMP_FKEYS2022_Benthic_Data02_CoralDemographics.csv") %>%
...
AR_ready_demo <- read_csv("data/FK2024_NCRMP_DRM_MIR_corsz_AR_NK.csv")
...
#RATIO ESTIMATION
ratio_est_2022 <- read_csv("data/fk2022_NCRMP_MIR_sppLbar_dom.csv")
ratio_est_2024 <- read_csv("data/fk2024_NCRMP_MIR_sppLbar_domv2.csv")
...
write_csv(a, "denisty_data.csv")
write_csv(b, "old_mort.csv")
write_csv(c, "max_diam.csv")
```
...
species_images <- list(
  "Acropora cervicornis" = list(path = "coral_image_files/staghorn.jpg", type = "jpg"),
  "Colpophyllia natans" = list(path = "coral_image_files/col_nata.jpg", type = "jpg"),
  "Dichocoenia stokesii" = list(path = "coral_image_files/dic_stok.jpg", type = "jpg"),
  "Diploria labyrinthiformis" = list(path = "coral_image_files/dip_laby.jpg", type = "jpg"),
  "Meandrina meandrites" = list(path = "coral_image_files/mea_mean.jpg", type = "jpg"),
  "Pseudodiploria clivosa" = list(path = "coral_image_files/pse_cliv.png", type = "png"),
  "Pseudodiploria strigosa" = list(path = "coral_image_files/pse_stri.png", type = "png"),
  "Montastraea cavernosa" = list(path = "coral_image_files/mon_cave.jpg", type = "jpg"),
  "Acropora palmata" = list(path = "coral_image_files/acr_palm.png", type = "png")
)
...
tmp_ble_2022 <- read_csv("data/fk2022_NCRMP_MIR_sppPVbar_Ball_dom.csv") %>%
...
tmp <- read_csv("data/fk2024_NCRMP_MIR_sppPVbar_Ball_dom.csv") %>%
    left_join(names_clean, by = "SPECIES_CD")
...
tmp_dis_2022 <- read_csv("data/fk2022_NCRMP_MIR_sppPVbar_Dall_dom.csv") %>%
    left_join(names_clean, by = "SPECIES_CD")
...
# Set path to data files
data_2024_path <- "data/FK2022_NCRMP_DRM_MIR_corsz_ARallv2.csv"
...
tmp_dis_2024 <- read_csv("data/fk2024_NCRMP_MIR_sppPVbar_Dall_dom.csv") %>%
    left_join(names_clean, by = "SPECIES_CD")
